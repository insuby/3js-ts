...
export const MeetOurTeam: FunctionComponent<{ name: string }> = memo(
  ({ name }) => {
        ...
        {[TEAM, ADVISORS].map((el, slideIndex) => {
          return (
            <div className="slide" key={`slide_${slideIndex}`}>
              {slideIndex === 1 && <span className="slide__go-to">team</span>}
              <div className="team">
                {el.map(
              ...
              {slideIndex === 0 && (
                <span className="slide__go-to">advisors</span>
              )}
            </div>
          );
        })}
