...
export const Roadmap: FunctionComponent<{ name: string }> = memo(({ name }) => {
  return (
    <Section name={name}>
      <div className="roadmap">
        ...
        <div className="roadmap__block">
          {data.map(({ quater, list }, index, arr) => {
            return (
              <div className="roadmap__slide" key={`${quater}-${index}`}>
                <div className={`roadmap__image roadmap__image_${index + 1}`} />
                <h4>{quater}</h4>
                <ul>
                  {list.map((item, index) => {
